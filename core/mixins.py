...
from core import exceptions, serializer_params
    ...
    def export(self, request, *args, **kwargs):
        if not self.export_fields:
            raise exceptions.ActionFailedException(
                cause='export fields must be declared')
        ...
        if not queryset.exists():
            raise exceptions.ReportNoRecordsException
        ...
        behavior = behaviors.ExportDataToCsvBehavior()
        behavior.export_fields = self.export_fields
        return behavior.to_response(file_name=csv_name, queryset=queryset)
