...
class MenuFilter(filterset.FilterSet):
    ...
    def filter_module(queryset, name, value):
        ids = models.ModuleMenu.objects.filter(module__id=value).values_list('menu__id', flat=True)
...
class ContentTypeFilter(filterset.FilterSet):
    ...
    app_label_not_in = CharInFilter(lookup_expr='in', field_name='model', exclude=True)

    @staticmethod
    def filter_app_label_model(queryset, name, value):
        return queryset.filter(Q(app_label__unaccent__icontains=value) | Q(model__unaccent__icontains=value))

    class Meta:
        model = auth_models.ContentType
        fields = ['app_label_in', 'app_label', 'model', 'app_label_or_model', 'app_label_not_in']


class ModuleMenuFilter(filterset.FilterSet):
    module = filterset.NumberFilter(lookup_expr=EXACT)
    module_description = filterset.CharFilter(lookup_expr=LIKE, field_name='module__description')
    menu = filterset.NumberFilter(lookup_expr=EXACT)
    menu_description = filterset.CharFilter(lookup_expr=LIKE, field_name='menu__description')
    root = filterset.NumberFilter(lookup_expr=EXACT)
    is_root = filterset.BooleanFilter(widget=BooleanWidget(), method='filter_is_root')
    root_or_menu_description = filterset.CharFilter(method='filter_root_menu_description')
    granted = filterset.BooleanFilter(widget=widgets.BooleanWidget())
    is_active = filterset.BooleanFilter(widget=widgets.BooleanWidget())
    ...
    def filter_root_menu_description(queryset, name, value):
        return queryset.filter(Q(root__description__icontains=value) | Q(menu__description__icontains=value))
...
class ModuleFilter(filterset.FilterSet):
    ...
    is_active = filterset.BooleanFilter(widget=widgets.BooleanWidget())
    granted = filterset.BooleanFilter(widget=widgets.BooleanWidget())
    id = filterset.NumberFilter(lookup_expr=EXACT)

    class Meta:
        model = models.Module
        fields = ['description', 'is_active', 'granted', 'id']
