...
from account.parameters import AccountParameters
from account.serializers import MyTokenObtainPairSerializer
from core import mixins as core_mixins


class AuthViewSetBase(viewsets.ModelViewSet, mixins.ViewSetExportMixin,
                      mixins.AuthAssociativeViewSetMixin, core_mixins.HistoryViewSetMixin):
    _permission = None
    ...
    @swagger_auto_schema(manual_parameters=AccountParameters.parameters_account)
...
class CustomModelViewSet(AuthViewSetBase, core_mixins.HistoryViewSetMixin):
...
class UserViewSet(mixins.AssociativeViewSetMixin, AuthViewSetBase):
    ...
    def associate_group(self, request, *args, **kwargs):
        ...
        actions.UserActions.associate_to_group(queryset=self.queryset,
                                               **rs.validated_data)
        return Response(data={'detail': True})
